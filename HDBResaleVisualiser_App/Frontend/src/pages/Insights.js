...
const Insights = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("📄 histRes raw:", rawHistText.slice(0, 200));
  ...
  // ⏱️ Unified timeline for all years
  const allYears = [...new Set([...historical.map(d => d.year), ...predictions.map(d => d.year)])];
