...
import { Button } from "@/components/ui/button"; // Adjust the Button component import if needed

const API_URL = "http://localhost:8000/api/"; // Adjust this URL based on your Django backend
...
export default function HDBResaleVisualizer() {
  ...
  // Fetch available towns from the Django backend
  useEffect(() => {
  ...
  // Fetch price trends or volatility data from the Django backend
      ...
      .then((res) => {
        ...
        setChartData({
          ...
          datasets: selectedTowns.map((town) => ({
            label: town,
            data: data.filter((d) => d.town === town).map((d) => d.avg_price || d.price_volatility),
